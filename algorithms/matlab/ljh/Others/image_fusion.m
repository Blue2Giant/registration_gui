function image_fusion(image_1,image_2,solution,save_dir)
...
if nargin < 4 || isempty(save_dir)
    save_dir = fullfile(pwd, 'save_image');
end
if ~exist(save_dir,'dir')
    mkdir(save_dir);
end
imwrite(fusion_image, fullfile(save_dir, 'fusion image.jpg'), 'jpg');

grid_num=7;
grid_size=floor(min(size(f_1,1),size(f_1,2))/grid_num);
[~,~,f_3]=mosaic_map(f_1,f_2,grid_size);
imwrite(f_3, fullfile(save_dir, 'Fused image of the board.jpg'), 'jpg');
