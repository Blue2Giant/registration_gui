...
edge_map = edgesDetect(gather(cat(3,image,image,image)), model.model);
try
    E = gpuArray(edge_map);
    E_Scalespace{1} = gather(E);
catch
    E_Scalespace{1} = edge_map;
end
...
if do_save
    for i=1:Layers
        imwrite(mat2gray(Nonelinear_Scalespace{i}), fullfile(save_dir, ['Nonelinear_Scalespace_' num2str(i) '.png']));
        imwrite(mat2gray(E_Scalespace{i}), fullfile(save_dir, ['E_Scalespace_' num2str(i) '.png']));
        imwrite(mat2gray(Max_Scalespace{i}), fullfile(save_dir, ['Max_Scalespace_' num2str(i) '.png']));
        imwrite(mat2gray(Min_Scalespace{i}), fullfile(save_dir, ['Min_Scalespace_' num2str(i) '.png']));
        imwrite(mat2gray(Phase_Scalespace{i}), fullfile(save_dir, ['Phase_Scalespace_' num2str(i) '.png']));
    end
end
