function [Bolb_space,Corner_space,Bolb_gradient_cell,Corner_gradient_cell,Bolb_angle_cell,Corner_angle_cell] = WSSF_gradient_feature(Scalespace, E_Scalespace, Max_Scalespace, Min_Scalespace, Phase_Scalespace, Scale_Invariance, nOctaves, save_dir)


    if nargin < 8
        save_dir = '';
    end
    do_save = ~isempty(save_dir);
    if do_save && ~exist(save_dir,'dir')
        mkdir(save_dir);
    end
   ...
   %sobel
    ...
    if do_save
        for j = 1:Layers
            imwrite(mat2gray(Bolb_space{j}), fullfile(save_dir, ['Bolb_space_' num2str(j) '.png']));
            imwrite(mat2gray(Corner_space{j}), fullfile(save_dir, ['Corner_space_' num2str(j) '.png']));
            imwrite(mat2gray(Bolb_gradient_cell{j}), fullfile(save_dir, ['Bolb_gradient_' num2str(j) '.png']));
            imwrite(mat2gray(Corner_gradient_cell{j}), fullfile(save_dir, ['Corner_gradient_' num2str(j) '.png']));
            imwrite(mat2gray(Bolb_angle_cell{j}), fullfile(save_dir, ['Bolb_angle_' num2str(j) '.png']));
            imwrite(mat2gray(Corner_angle_cell{j}), fullfile(save_dir, ['Corner_angle_' num2str(j) '.png']));
        end
    end
end
